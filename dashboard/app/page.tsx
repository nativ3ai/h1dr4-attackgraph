  ...
  return (
    <main className="shell">
      <header className="topbar">
        <div className="brand"><span className="brand-mark">H1</span><div><strong>H1DR4</strong><span>ATTACKGRAPH</span></div></div>
        ...
        <div className={`system-state ${connection}`}><span className="pulse" /><div><strong>{connection === 'live' ? 'SIBYL ONLINE' : connection === 'connecting' ? 'CONNECTING' : 'PREVIEW MODE'}</strong><span>{connection === 'live' ? `Memory synchronized · ${formatTime(snapshot.memory.last_event)} UTC` : 'Waiting for local memory'}</span></div></div>
      </header>
      ...
      <div className="workspace">
        ...
        <section className="graph-panel">
          <div className="panel-heading">
            <div><span>{view === 'graph' ? 'LIVE TOPOLOGY' : 'SIBYL EVENT LOG'}</span><h1>{view === 'graph' ? 'Attack surface' : 'Engagement timeline'}</h1></div>
            ...
            <div className="graph-canvas">
              ...
              <div className="graph-stage" style={{ transform: `scale(${zoom})` }}>
                ...
                {snapshot.nodes.map((node) => (
                  <button className={`graph-node ${node.kind} ${selectedNode?.id === node.id ? 'selected' : ''}`} key={node.id} style={{ left: `${node.x}%`, top: `${node.y}%` }} type="button" onClick={() => setSelectedNode(node)}>
                    <span className="node-core" /><strong>{node.label}</strong><small>{node.meta}</small>
                  </button>
                ))}
              </div>
              <div className="graph-legend"><span><i className="legend-confirmed" /> Confirmed</span><span><i className="legend-hypothesis" /> Hypothesis</span><span><i className="legend-regression" /> Regression</span></div>
              {selectedNode && <NodeInspector node={selectedNode} onClose={() => setSelectedNode(null)} />}
            </div>
          ) : <CentralTimeline events={snapshot.events} />}

          <div className="memory-directive"><span>AGENT DIRECTIVE</span><p>{snapshot.brief.agent_instruction}</p></div>
        </section>
      ...
      <footer className="statusbar">
        ...
        <span>TENANT <strong>{snapshot.engagement.operator_id?.toUpperCase() || 'ISOLATED'}</strong></span>
        <span className="status-spacer" />
        {engagementId && <a href={`/api/engagements/${engagementId}/export`}>EXPORT JSON</a>}
        <span>LAST EVENT <strong>{formatTime(snapshot.memory.last_event)} UTC</strong></span>
...
function IntelItem({ title, meta }: { title: string; meta: string }) { return <div className="intel-item"><i /><div><strong>{title}</strong><small>{meta}</small></div></div>; }
...
function ActionCard({ action, onReview }: { action: Action | null; onReview: () => void }) {
  return <section className="action-card"><div className="section-label"><span>ACTION QUEUE</span><b>{action ? '1' : '0'}</b></div>{action ? <><div className="action-id"><span>{action.id.toUpperCase()}</span><em>{action.status.replaceAll('_', ' ').toUpperCase()}</em></div><h2>{action.purpose}</h2><code>{action.command}</code><dl><div><dt>Lane</dt><dd>{action.lane.toUpperCase()}</dd></div><div><dt>Runtime</dt><dd>≤ {action.max_minutes} MIN</dd></div><div><dt>Budget</dt><dd>{action.budget_usdc.toFixed(2)} USDC</dd></div></dl><button type="button" className="review-action" onClick={onReview}>REVIEW ACTION</button></> : <EmptyState text="No actions awaiting review" />}</section>;
}
...
function NodeInspector({ node, onClose }: { node: GraphNode; onClose: () => void }) {
  return <aside className="node-inspector"><button type="button" onClick={onClose} aria-label="Close node details">×</button><span>{node.meta}</span><h2>{node.label}</h2><pre>{JSON.stringify(node.detail, null, 2)}</pre></aside>;
}
